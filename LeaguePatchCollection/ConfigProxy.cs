...
public partial class ConfigProxy
{
    ...
    public static string? LcuNavUrl { get; private set; }
        ...
        try
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                if (serverClient == null || !serverClient.Connected)
                {
                    ...
                    await sslStream.AuthenticateAsClientAsync(new() { TargetHost = targetHost }, token);
                }
    ...
    private static byte[] ModifyResponsePayload(byte[] payload, string endpoint)
    {
        ...
        if (LeaguePatchCollectionUX.SettingsManager.ConfigSettings.Novgk)
        {
            ...
            SetKey(configObject, "lol.client_settings.clash.vanguard_check_enabled", false);
        ...
        {
            if (configObject is JsonObject root)
            {
                foreach (var kvp in root)
                {
                    string key = kvp.Key;

                    if (key.EndsWith(".operational.legacy.AccountVerification"))
                    {
                        SetNestedKeys(configObject, key, "IsNewAvsServiceEnabled", false);
                        SetNestedKeys(configObject, key, "Enabled", false);
                        SetNestedKeys(configObject, key, "PhoneValidationEnabled", false);
                        SetNestedKeys(configObject, key, "SettingsEnabled", false);
                        SetNestedKeys(configObject, key, "ShouldUseNewAvs", false);
                    }
                    if (key.EndsWith(".operational.legacy.Challenges"))
                    {
                        SetNestedKeys(configObject, key, "ClientState", "Hidden");
                    }
                    /*if (key.EndsWith(".operational.legacy.ClashConfig"))
                    {
                        SetNestedKeys(configObject, key, "Visibility", "Hidden");
                        SetNestedKeys(configObject, key, "EnabledState", "Disabled");
                    }*/
                    if (key.EndsWith(".operational.legacy.LcuPurchaseWidget"))
                    {
                        SetNestedKeys(configObject, key, "NonRefundableDisclaimerEnabled", false);
                    }
                    if (key.EndsWith(".operational.legacy.LeagueConfig"))
                    {
                        SetNestedKeys(configObject, key, "ApexDemotionNotificationEnabled", false);
                        SetNestedKeys(configObject, key, "EosNotificationsEnabled", false);
                        SetNestedKeys(configObject, key, "IsGlobalNotificationsEnabled", false);
                        SetNestedKeys(configObject, key, "RankedReferenceModalEnabled", false);
                    }
                    if (key.EndsWith(".operational.legacy.PlayerBehavior"))
                    {
                        SetNestedKeys(configObject, key, "ReformCardV2Enabled", false);
                        SetNestedKeys(configObject, key, "CodeOfConductEnabled", false);
                    }
                    if (key.EndsWith(".operational.legacy.Inventory"))
                    {
                        SetNestedKeys(configObject, key, "Enabled", false);
                        SetNestedKeys(configObject, key, "BaseServiceUrl", "");
                    }
                    if (key.EndsWith(".operational.legacy.PlayerFeedbackTool"))
                    {
                        SetNestedKeys(configObject, key, "BackendUrl", "");
                        SetNestedKeys(configObject, key, "EnableHomeTrigger", false);
                    }
                    if (key.EndsWith(".operational.legacy.Missions"))
                    {
                        SetNestedKeys(configObject, key, "MissionsFrontEndEnabled", false);
                    }
                    if (key.EndsWith(".operational.legacy.LcuChampionSelect") || key.EndsWith(".operational.legacy.LcuLobby"))
                    {
                        SetNestedKeys(configObject, key, "PotatoModeForced", true);
                    }
                    if (key.EndsWith(".operational.legacy.PlayerNotification"))
                    {
                        SetNestedKeys(configObject, key, "Enabled", false);
                    }
                }
            ...
            if (configObject?["lol.client_settings.nacho.active_banners"] is JsonObject activeBannersConfig && activeBannersConfig.ContainsKey("enabled"))
            {
                activeBannersConfig["enabled"] = false;
            }

            SetKey(configObject, "lol.client_settings.loot.enable_mythic_shoppefront", false);
        ...
        SetKey(configObject, "lol.client_settings.client_navigability.base_url", $"http://127.0.0.1:{LeagueProxy.LcuNavPort}");
        SetKey(configObject, "lol.client_settings.player_platform_edge.url", $"http://127.0.0.1:{LeagueProxy.PlatformPort}");
        SetKey(configObject, "lol.client_settings.remedy.obfuscated_player_name", false);
        ...
        SetNestedKeys(configObject, "lol.client_settings.opmon_relay", "isEnabled", false);
        SetNestedKeys(configObject, "lol.client_settings.opmon_relay", "sessionSamplePercent", 0);
    ...
    public static void SetNestedKeys(JsonNode? configObject, string parentKey, string childKey, object value)
    {
